...
module sub_mod_nl_lemmas {
    ...
    lemma refine_int64_view(lh: uint32, uh: uint32, full: int64) returns (r: int64_view_t)
        requires lh == dw_lh(to_2s_complement_bv64(full));
        requires uh == dw_uh(to_2s_complement_bv64(full));
    ...
    {
        && A.lh == dw_lh(to_2s_complement_bv64(A.full))
        && A.uh == dw_uh(to_2s_complement_bv64(A.full))
    ...
    {
      LemmaDivBounded(x, Power2.Pow2(shift));
      x / Power2.Pow2(shift)
    }
