...
module MultiplyExpr128 {
            ...
            interp_half(a) * interp_half(b);
    {
        calc == {
            ...
            {
                assume flh(t1) * B * B + fuh(t1) * B * B * B == t1 * B * B;
            }
            flh(p0) + flh(t0) * B + t1 * B * B + fuh(p3) * B * B * B;
            {
                assert t1 == fuh(p1) + fuh(p2) + flh(p3) + fuh(t0);
            }
            ...
            {
                assume flh(p3) * B * B + fuh(p3) * B * B * B == p3 * B * B; 
            }
            flh(p0) + flh(t0) * B + fuh(p1) * B * B + fuh(p2) * B * B + fuh(t0) * B * B + p3 * B * B;
            {
                assume flh(t0) * B + fuh(t0) * B * B == t0 * B; 
            }
            flh(p0) + fuh(p1) * B * B + fuh(p2) * B * B + t0 * B + p3 * B * B;
            {
                assert t0 == fuh(p0) + flh(p1) + flh(p2);
            }
            flh(p0) + fuh(p1) * B * B + fuh(p2) * B * B + (fuh(p0) + flh(p1) + flh(p2)) * B + p3 * B * B;
            flh(p0) + fuh(p1) * B * B + fuh(p2) * B * B + fuh(p0) * B + flh(p1) * B + flh(p2) * B + p3 * B * B;
            {
                assume fuh(p2) * B * B + flh(p2) * B == p2 * B;
            }
            flh(p0) + fuh(p1) * B * B + p2 * B + fuh(p0) * B + flh(p1) * B + p3 * B * B;
            {
                assume fuh(p1) * B * B + flh(p1) * B == p1 * B;
            }
            flh(p0) + p1 * B + p2 * B + fuh(p0) * B + p3 * B * B;
            {
                assume fuh(p0) * B + flh(p0) == p0;
            }
            p0 + p1 * B + p2 * B + p3 * B * B;
        }
