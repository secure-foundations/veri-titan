...
module rv_machine {
   ...
   /* memory definitions */ 
        ...
        ok: bool)
    {
        ...
        function method eval_ins32(ins: ins32): state
        {
            if !ok then
                this
            else match ins
              case _ => this // TODO: executable semantics
        }
        ...
        function eval_cond(c:cond): bool
        {
            eval_cmp(c)
        }

        function method eval_while(condition: cond, body: code, n: nat): state
            decreases body, n
        {
            if !ok then
                this
            else
              exists n:nat :: eval_while(condition, body, n)
              
                if n == 0 then
                    this
                else
                  eval_code(body).eval_while(condition, body, n - 1)
        }

        function method eval_cmp(cond: cond): bool
        {
            var Cmp(op, r1, r2) := cond;
            match op 
                case Eq => (read_reg32(r1) == read_reg32(r2))
                case Ne => (read_reg32(r1) != read_reg32(r2))
                case Le => (read_reg32(r1) <= read_reg32(r2))
                case Lt => (read_reg32(r1) < read_reg32(r2))
                case Gt => (read_reg32(r1) > read_reg32(r2))
                case Ge => (read_reg32(r1) >= read_reg32(r2))
        }

        function method eval_if_else(cond: cond, ifT: code, ifF: code): state
            ...
            decreases c, 0
        {
            match c
                ...
                case While(cond, body) => eval_while(cond, body)
