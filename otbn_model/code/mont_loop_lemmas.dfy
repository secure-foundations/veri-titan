...
module mont_loop_lemmas {
        ...
        ensures cong_B256(w25, (a0 + y0 * xi) * m0d);
    {
        calc ==> {
            ...
            cong_B256(a0 + y0 * xi, p1.lh + p1.uh * NT.BASE_256);
                {
                    // cong_add_lemma_4(p1.lh, p1.uh, NT.BASE_256);
                    reveal cong();
                }
            cong_B256(a0 + y0 * xi, p1.lh);
                { reveal cong(); }
            cong_B256(p1.lh, a0 + y0 * xi);
                // { cong_mul_lemma_1(p1.lh, a0 + y0 * xi, m0d, NT.BASE_256); }
        ...
        calc ==> {
            ...
            cong_B256(w25 + w26 * NT.BASE_256, (a0 + y0 * xi) * m0d);
                {
                    cong_add_lemma_4(w25, w26, NT.BASE_256);
                    assert cong_B256(w25, w25 + w26 * NT.BASE_256);
                    reveal cong();
                }
        ...
        ensures p2.lh == 0;
    {
        ...
        assert cong_B256(ui * m0, -p1_full) by {
            assert cong_B256(m0d * m0 * p1.full, -p1_full) by {
                // cong_mul_lemma_1(m0d * m0, -1, p1.full, NT.BASE_256);
            }
            assert cong_B256(ui * m0, p1.full * m0d * m0) by {
                // cong_mul_lemma_1(ui, p1.full * m0d, m0, NT.BASE_256);
            }
            reveal cong(); 
        }
        ...
        calc ==> {
            ...
            { cong_add_lemma_1(ui * m0, - p1_full, p1.lh, NT.BASE_256); }
        ...
        calc ==> {
            ...
            { cong_add_lemma_5(ui * m0 + p1.lh, p2.lh + p2.uh * NT.BASE_256, -(p2.uh as int), NT.BASE_256); }
            cong_B256(ui * m0 + p1.lh, p2.lh);
            { cong_trans_lemma(p2.lh, ui * m0 + p1.lh, 0, NT.BASE_256); }
            cong_B256(p2.lh, 0);
        }

        assert cong_B256(p2.lh, 0);
        cong_residual_lemma(p2.lh, 0, NT.BASE_256);
    }
        ...
        ensures mont_loop_inv(xi, ui, p1, p2, y, m, a, a, 1)
    {
        assert cong_B256(m0d * m[0], -1) by {
            lsw_cong_lemma(m);
            // cong_mul_lemma_1(to_nat(m), m[0], m0d, NT.BASE_256);
            reveal cong();
        }
        ...
        ensures mont_loop_inv(xi, ui, next_p1, next_p2, y, m, prev_a, next_a, j+1);
    {
        ...
        assert next_p1.lh + next_p1.uh * NT.BASE_256 == p1_uh + y_j * xi + a_j by {
            uint512_view_lemma(next_p1);
        }

        assert next_p2.lh + next_p2.uh * NT.BASE_256 == m_j * ui + next_p1.lh + p2_uh by {
        ...
        assert ea_nat' == ea_nat + next_p2.lh * pow_B256_j by {
            ...
            assert to_nat([0] + a[..j-1]) == to_nat(a[..j-1]) * NT.BASE_256 by {
                to_nat_zero_prepend_lemma(a[..j-1]);
            }

            assert to_nat([0] + next_a[..j]) == to_nat(next_a[..j]) * NT.BASE_256 by {
    ...
    lemma mont_loop_inv_post_lemma(
        ...
        ensures to_nat(a) * NT.BASE_256 + bout * pow_B256(NUM_WORDS+1)
            == xi * to_nat(y) + ui * to_nat(m) + to_nat(prev_a);
    {
        // var m := to_nat(m);
        calc {
            to_nat(a) * NT.BASE_256 + bout * pow_B256(NUM_WORDS+1);
                { reveal to_nat(); }
            (to_nat(a[..NUM_WORDS-1]) + a[NUM_WORDS-1] * pow_B256(NUM_WORDS-1)) * NT.BASE_256 + bout * pow_B256(NUM_WORDS+1);
            to_nat(a[..NUM_WORDS-1]) * NT.BASE_256 + a[NUM_WORDS-1] * pow_B256(NUM_WORDS-1) * NT.BASE_256 + bout * pow_B256(NUM_WORDS+1);
            to_nat(a[..NUM_WORDS-1]) * NT.BASE_256 + a[NUM_WORDS-1] * pow_B256(NUM_WORDS) + bout * pow_B256(NUM_WORDS+1);
            to_nat(a[..NUM_WORDS-1]) * NT.BASE_256 + a[NUM_WORDS-1] * pow_B256(NUM_WORDS) + bout * NT.BASE_256 * pow_B256(NUM_WORDS);
            to_nat(a[..NUM_WORDS-1]) * NT.BASE_256 + p2.uh * pow_B256(NUM_WORDS) + p1.uh * pow_B256(NUM_WORDS);
    ...
    lemma mont_loop_cond_sub_borrow_lemma(xi: uint256,
        ...
        requires to_nat(a) * NT.BASE_256 + pow_B256(NUM_WORDS+1)
        ...
        ensures cong(to_nat(next_a) * NT.BASE_256, xi * to_nat(y) + ui * m + prev_a, m)
    {
        if to_nat(a) >= m {
        ...
        calc {
            to_nat(next_a) * NT.BASE_256;
            to_nat(a) * NT.BASE_256 + pow_B256(NUM_WORDS) * NT.BASE_256 - m * NT.BASE_256;
                { power_add_one_lemma(NT.BASE_256, NUM_WORDS); }
            to_nat(a) * NT.BASE_256 + pow_B256(NUM_WORDS+1) - m * NT.BASE_256;
            xi * to_nat(y) + ui * m + prev_a - m * NT.BASE_256;
        }
        
        calc ==> {
            true;
                { reveal cong(); }
            cong(to_nat(next_a) * NT.BASE_256, xi * to_nat(y) + ui * m + prev_a - m * NT.BASE_256, m);
                {
                    cong_add_lemma_5(to_nat(next_a) * NT.BASE_256,
                        xi * to_nat(y) + ui * m + prev_a - m * NT.BASE_256, NT.BASE_256, m);
                    assert xi * to_nat(y) + ui * m + prev_a - m * NT.BASE_256 + NT.BASE_256 * m == 
                        xi * to_nat(y) + ui * m + prev_a;
                }
            cong(to_nat(next_a) * NT.BASE_256, xi * to_nat(y) + ui * m + prev_a, m);
        }
    }
    ...
    lemma mont_loop_cond_sub_lemma(
        ...
        requires |next_a| == |y| == NUM_WORDS;
        requires prev_a < m + to_nat(y);
        requires to_nat(a) * NT.BASE_256 + bout * pow_B256(NUM_WORDS+1)
            == xi * to_nat(y) + ui * m + prev_a;
        requires bout == 0 ==>
        ...
        ensures cong(to_nat(next_a) * NT.BASE_256, xi * to_nat(y) + ui * m + prev_a, m)
    {
        assert to_nat(a) + bout * pow_B256(NUM_WORDS) < to_nat(y) + m by {
            calc {
                (to_nat(a) + bout * pow_B256(NUM_WORDS)) * NT.BASE_256;
                to_nat(a) * NT.BASE_256 + bout * pow_B256(NUM_WORDS) * NT.BASE_256;
                to_nat(a) * NT.BASE_256 + bout * pow_B256(NUM_WORDS+1);
                xi * to_nat(y) + ui * m + prev_a;
                <
                ...
                NT.BASE_256 * to_nat(y) + (ui + 1) * m;
                <= { assert ui + 1 <= NT.BASE_256; }
                NT.BASE_256 * to_nat(y) + NT.BASE_256 * m;
                (to_nat(y) + m) * NT.BASE_256;
            }
        }
        ...
        if bout == 1 {
            ...
            reveal cong();
            assert cong(to_nat(next_a) * NT.BASE_256,
        ...
        ensures montmul_inv(a, x, i+1, y, rsa);
    {
        ...
        calc ==> {
            ...
            cong(curr_a * pow_B256(1) * pow_B256(i), x[i] * to_nat(y) * pow_B256(i) + prev_a * pow_B256(i), rsa.M);
                {
                    power_add_one_lemma(NT.BASE_256, i);
